...
//    vec3 specularFinal = specularConstant * material.specular;
    ...
    vec3 diffuseFinal =(diffuseLight * diffuse);

    vec3 lightColor = ambientLight + diffuseFinal;

    vec4 tex1 = texture2D(u_texture0, v_texcoord);
    //vec4 tex2 = texture2D(u_texture1, v_texcoord/0.2);
    //vec4 texColor = tex1 * tex2;
    color =tex1 * vec4(lightColor, 1.0f);/** (vec4(ambientLight, 1.f) + vec4(diffuseFinal, 1.0f) + vec4(specularFinal, 1.f))*/
}
